...
@section('title', app_name() . ' | ' . __('labels.backend.access.cloth.management'))
        ...
        <div class="row">
            <div class="col-sm-5">
                <h4 class="card-title mb-0">
                    {{ __('labels.backend.access.cloth.management') }}
                </h4>
            </div><!--col-->
            ...
            <div class="col">
                <div class="table-responsive">
                    <table class="table table-striped data-table">
                        <thead>
                        <tr>
                            {{-- <th>@lang('labels.backend.access.cloth.table.id')</th> --}}
                            <th>@lang('labels.backend.access.cloth.table.name')</th>
                            <th>@lang('labels.backend.access.cloth.table.created')</th>
                            <th>@lang('labels.backend.access.cloth.table.last_updated')</th>
        ...
        <div class="row">
            <div class="col-7">
                <div class="float-left">
                    {!! $colors->total() !!} {{ trans_choice('labels.backend.access.cloth.table.total', $colors->total()) }}
                </div>
            </div><!--col-->
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="createTagLabel">@lang('labels.backend.access.cloth.create')</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>

    <form autocomplete="off" method="POST" action="{{ route('admin.product.cloth.store') }}">
       {{ csrf_field() }}
      <div class="modal-body">
          <div class="form-group">
            <label for="name" class="col-form-label">@lang('labels.backend.access.cloth.table.name'):</label>
...
<div class="modal fade" id="editTag" tabindex="-1" role="dialog" aria-labelledby="createTagLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="createTagLabel">@lang('labels.backend.access.cloth.edit')</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>

    <form autocomplete="off" method="POST" action="{{ route('admin.product.cloth.update', 'test') }}">
        {{method_field('patch')}}
        {{ csrf_field() }}
      <div class="modal-body">
          <div class="form-group">
