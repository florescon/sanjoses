...
@section('title', app_name() . ' | ' . __('labels.backend.access.unit.management'))
            ...
            <div class="col">
                <div class="table-responsive">
                    <table class="table table-striped data-table">
                        <thead>
                        <tr>
                            {{-- <th>@lang('labels.backend.access.unit.table.id')</th> --}}
                            <th>@lang('labels.backend.access.unit.table.name')</th>
                            <th>@lang('labels.backend.access.unit.table.created')</th>
                            <th>@lang('labels.backend.access.unit.table.last_updated')</th>
        ...
        <div class="row">
            <div class="col-7">
                <div class="float-left">
                    {!! $colors->total() !!} {{ trans_choice('labels.backend.access.unit.table.total', $colors->total()) }}
                </div>
            </div><!--col-->
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="createTagLabel">@lang('labels.backend.access.unit.create')</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>

    <form autocomplete="off" method="POST" action="{{ route('admin.product.unit.store') }}">
       {{ csrf_field() }}
      <div class="modal-body">
          <div class="form-group">
            <label for="name" class="col-form-label">@lang('labels.backend.access.unit.table.name'):</label>
...
<div class="modal fade" id="editTag" tabindex="-1" role="dialog" aria-labelledby="createTagLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="createTagLabel">@lang('labels.backend.access.unit.edit')</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>

    <form autocomplete="off" method="POST" action="{{ route('admin.product.unit.update', 'test') }}">
        {{method_field('patch')}}
        {{ csrf_field() }}
      <div class="modal-body">
          <div class="form-group">
